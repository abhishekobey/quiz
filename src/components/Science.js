...
const Science = () => {
    ...
    const next = () => {
        if (prevClick === 0)
        {
            ...
            if (currentQues === SciQues.length-1)
            {
                setShowsubmit(true)
            }
            else { setShowsubmit(false) }

            if (nextQues<SciQues.length)
            {
        ...
        <div>
            {showScore ? (
                <div className='score'>
                    ...
                    <h1 style={ yellowColor }>Number of Questions Missed : {SciQues.length - Attempted}</h1>
                    <h1 style={ redColor }>Number of Questions Wrong : {SciQues.length - (SciQues.length-Attempted) - score}</h1>
                ...
                <>  
                    ...
                    <h3>Q{`${SciQues[currentQues].no}`} : {`${SciQues[currentQues].ques}`} </h3><br/>
                    <div className='options' >
                        {ManageAns ? (
                            <div className='after_click'>
                                {changeGreenColor ? (
                                <div className='changeGreenColor'>
                                    {SciQues[currentQues].options.map((option) => (<button>{option.option}</button>))}
                                </div>
                                ): changeRedColor ? (
                                <div className='changeRedColor'>
                                    {SciQues[currentQues].options.map((option) => (<button>{option.option}</button>))}
                                </div>
                                ):(<div>{SciQues[currentQues].options.map((option) => (<button>{option.option}</button>))}</div>)}
                            </div>
                        ):(
                            <div className='nochange'>
                                {SciQues[currentQues].options.map((option) => (<button onClick= {()=> handleAns(option.isCorrect)}>{option.option}</button>))}
                            </div>
                        )}
