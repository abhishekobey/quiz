...
import PlantsQues from '../PlantsQues'
...
const Plants = () => {
    ...
    const next = () => {
        if (prevClick === 0)
        {
            ...
            if (currentQues === PlantsQues.length-1)
            {
                setShowsubmit(true)
            }
            else { setShowsubmit(false) }

            if (nextQues<PlantsQues.length)
            {
        ...
        <div>
            {showScore ? (
                <div className='score'>
                    ...
                    <h1 style={ yellowColor }>Number of Questions Missed : {PlantsQues.length - Attempted}</h1>
                    <h1 style={ redColor }>Number of Questions Wrong : {PlantsQues.length - (PlantsQues.length-Attempted) - score}</h1>
                ...
                <>  
                    ...
                    <h3>Q{`${PlantsQues[currentQues].no}`} : {`${PlantsQues[currentQues].ques}`} </h3><br/>
                    <div className='options' >
                        {ManageAns ? (
                            <div className='after_click'>
                                {changeGreenColor ? (
                                <div className='changeGreenColor'>
                                    {PlantsQues[currentQues].options.map((option) => (<button>{option.option}</button>))}
                                </div>
                                ): changeRedColor ? (
                                <div className='changeRedColor'>
                                    {PlantsQues[currentQues].options.map((option) => (<button>{option.option}</button>))}
                                </div>
                                ):(<div>{PlantsQues[currentQues].options.map((option) => (<button>{option.option}</button>))}</div>)}
                            </div>
                        ):(
                            <div className='nochange'>
                                {PlantsQues[currentQues].options.map((option) => (<button onClick= {()=> handleAns(option.isCorrect)}>{option.option}</button>))}
                            </div>
                        )}
