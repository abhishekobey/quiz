...
const Tech = () => {
    ...
    const next = () => {
        if (prevClick === 0)
        {
            ...
            if (currentQues === TechQues.length-1)
            {
                setShowsubmit(true)
            }
            else { setShowsubmit(false) }

            if (nextQues<TechQues.length)
            {
        ...
        <div>
            {showScore ? (
                <div className='score'>
                    ...
                    <h1 style={ yellowColor }>Number of Questions Missed : {TechQues.length - Attempted}</h1>
                    <h1 style={ redColor }>Number of Questions Wrong : {TechQues.length - (TechQues.length-Attempted) - score}</h1>
                ...
                <>  
                    ...
                    <h3>Q{`${TechQues[currentQues].no}`} : {`${TechQues[currentQues].ques}`} </h3><br/>
                    <div className='options' >
                        {ManageAns ? (
                            <div className='after_click'>
                                {changeGreenColor ? (
                                <div className='changeGreenColor'>
                                    {TechQues[currentQues].options.map((option) => (<button>{option.option}</button>))}
                                </div>
                                ): changeRedColor ? (
                                <div className='changeRedColor'>
                                    {TechQues[currentQues].options.map((option) => (<button>{option.option}</button>))}
                                </div>
                                ):(<div>{TechQues[currentQues].options.map((option) => (<button>{option.option}</button>))}</div>)}
                            </div>
                        ):(
                            <div className='nochange'>
                                {TechQues[currentQues].options.map((option) => (<button onClick= {()=> handleAns(option.isCorrect)}>{option.option}</button>))}
                            </div>
                        )}
